...
umbral_golpe = 2.2  
umbral_caricia = 0.85              # Umbral mínimo de voltaje para detectar un pico como golpe
tiempo_entre_picos = 100       # Distancia mínima (en muestras) entre dos picos consecutivos

OFFSET = 0.58                  # Offset usado para normalizar el voltaje
...
def generar_caricia(df):
    ...
    picos, _ = find_peaks(voltajes, height=umbral_caricia, distance=tiempo_entre_picos)
    ...
    for pico in picos:
        for offset in [-7, -5, -3, 0]:
            ...
            if inicio >= 0 and fin <= len(voltajes):
                ...
                if etiqueta_mayoritaria != "caricia_dedo" or repes / len(etiquetas_segmento) < 0.7:
                    continue  # Saltar si no es caricia clara
...
def generar_golpes(df):
    ...
    picos, _ = find_peaks(voltajes, height=umbral_golpe, distance=tiempo_entre_picos)
